...
class SimpleTransportComponent(Component):
    ...
    """
    _play_button = ButtonControl(color='Transport.PlayOff')
    _stop_button = ButtonControl(color='Transport.PlayOff')
    _record_button = ButtonControl(color='Transport.PlayOff')
    _metronome_button = ButtonControl(color='Transport.PlayOff')
    ...
    def _on_playing_changed(self):
        self._play_button.color = 'Transport.PlayOn' if self.song.is_playing else 'Transport.PlayOff'
        self._stop_button.color = 'Transport.StopOff' if self.song.is_playing else 'Transport.StopOn'

    @listens('record_mode')
    def _on_record_mode_changed(self):
        self._record_button.color = 'Transport.RecordOn' if self.song.record_mode else 'Transport.RecordOff'

    @listens('metronome')
    def _on_metronome_changed(self):
        self._metronome_button.color = 'Transport.MetronomeOn' if self.song.metronome else 'Transport.MetronomeOff'
...
class SimpleMixerComponent(Component):
    ...
    _mute_button = ButtonControl(color='Transport.PlayOff', pressed_color='Transport.PlayOff')
    _solo_button = ButtonControl(color='Transport.PlayOff', pressed_color='Transport.PlayOff')
    _arm_button = ButtonControl(color='Transport.PlayOff', pressed_color='Transport.PlayOff')
    _cue_volume_encoder = EncoderControl()
    _shift_button = ButtonControl(color='Transport.PlayOff', pressed_color='Transport.PlayOff')
    ...
    def _on_volume_changed(self, value, encoder):
        ...
        if track and track != self.song.master_track:
            multiplier = 0.001 if self._shift_button.is_pressed else 0.01
    ...
    def _on_pan_changed(self, value, encoder):
        track = self._get_selected_track()
        if track and track != self.song.master_track:
            multiplier = 0.002 if self._shift_button.is_pressed else 0.02
            delta = value * multiplier
    ...
    def _on_solo_changed(self):
        track = self.song.view.selected_track
        if track and track != self.song.master_track:
            self._solo_button.color = 'Transport.LoopOn' if track.solo else 'Transport.PlayOff' # Blue when solo
        else:
            self._solo_button.color = 'Transport.PlayOff'

    @listens('mute')
    def _on_mute_changed(self):
        track = self.song.view.selected_track
        if track and track != self.song.master_track:
            self._mute_button.color = 'Transport.StopOn' if track.mute else 'Transport.PlayOff' # Yellow when muted
        else:
            self._mute_button.color = 'Transport.PlayOff'

    @listens('arm')
    def _on_arm_changed(self):
        track = self.song.view.selected_track
        if track and track.can_be_armed:
            self._arm_button.color = 'Transport.RecordOn' if track.arm else 'Transport.PlayOff' # Red when armed
        else:
            self._arm_button.color = 'Transport.PlayOff'
...
class CustomControlComponent(Component):
    ...
    def _on_scroll_v(self, value, encoder):
        try:
            ...
            direction = Live.Application.Application.View.NavDirection.down if value > 0 else Live.Application.Application.View.NavDirection.up
            app_view.scroll_view(direction, 'Session', False)
        except Exception:
            pass
    ...
    def _on_scroll_h(self, value, encoder):
        try:
            app_view = Live.Application.get_application().view
            direction = Live.Application.Application.View.NavDirection.right if value > 0 else Live.Application.Application.View.NavDirection.left
            app_view.scroll_view(direction, 'Session', False)
        except Exception:
            pass
